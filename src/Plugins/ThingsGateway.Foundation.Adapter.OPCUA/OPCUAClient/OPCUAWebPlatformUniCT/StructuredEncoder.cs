...
namespace ThingsGateway.Foundation.Adapter.OPCUA
{
    public class StructuredEncoder
    {
        ...
        private void EncodeBoolean(JToken jToken, string fieldName, int lenghtOfArray)
        {
            if (lenghtOfArray == 1)
            {
                if (jToken.Type != JTokenType.Boolean)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be a Boolean but received a " + jToken.Type.ToString());
                _be.WriteBoolean(fieldName, jToken.ToObject<Boolean>());
            }
            else
            {
                if (jToken.Type != JTokenType.Array)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array but received a " + jToken.Type.ToString());
                JToken[] jtArray = jToken.Children().ToArray();
                if (jtArray.Length != lenghtOfArray)
                    throw new Exception("Wrong Object Properties: The Array property named " + fieldName + " should be an Array with lenght = " + lenghtOfArray);
                if (jtArray.GetElementsType() != JTokenType.Boolean)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array of JSON Boolean");
        ...
        private void EncodeByte(JToken jToken, string fieldName, int lenghtOfArray)
        {
            if (lenghtOfArray == 1)
            {
                if (jToken.Type != JTokenType.Integer)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be a Integer but received a " + jToken.Type.ToString());
                Byte valueToWrite;
                try
                {
                ...
                {
                    throw new Exception("Range Error: " + exc.Message);
                }
            ...
            else
            {
                ...
                if (jtArray.GetElementsType() != JTokenType.Integer)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array of Integer");
        ...
        private void EncodeByteString(JToken jToken, string fieldName, int lenghtOfArray)
        {
            if (lenghtOfArray == 1)
            {
                if (jToken.Type != JTokenType.String)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be a String but received a " + jToken.Type.ToString());
            ...
            else
            {
                ...
                if (jtArray.GetElementsType() != JTokenType.String)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array of JSON String");
        ...
        private void EncodeDateTime(JToken jToken, string fieldName, int lenghtOfArray)
        {
            if (lenghtOfArray == 1)
            {
                if (jToken.Type != JTokenType.Date)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be a Date but received a " + jToken.Type.ToString());
            ...
            else
            {
                ...
                if (jtArray.GetElementsType() != JTokenType.Date)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array of Date");
                ...
                {
                    throw new Exception("One or more Strings in the Array not formatted correctly. " + exc.Message);
                }
        ...
        private void EncodeDouble(JToken jToken, string fieldName, int lenghtOfArray)
        {
            if (lenghtOfArray == 1)
            {
                if (jToken.Type != JTokenType.Float && jToken.Type != JTokenType.Integer)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be a Number but received a " + jToken.Type.ToString());
                Double valueToWrite;
                try
                {
            ...
            else
            {
                ...
                if (jtArray.Length != lenghtOfArray)
                    throw new Exception("Wrong Object Properties: The Array property named " + fieldName + " should be an Array with lenght = " + lenghtOfArray);
                if (jtArray.GetElementsType() != JTokenType.Integer)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array of Number");
        ...
        private void EncodeExpandedNodeId(JToken jToken, string fieldName, int lenghtOfArray)
        {
            if (lenghtOfArray == 1)
            {
                ...
                if (jToken.Type != JTokenType.Object)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Object but received a " + jToken.Type.ToString());
                JObject jObject = jToken.ToObject<JObject>();
                if (!jObject.ContainsKey("NodeId") || jObject["NodeId"].Type != JTokenType.String)
                    throw new Exception("The Object " + fieldName + " must have the string Property \"NodeId\"");
                if (!jObject.ContainsKey("NamespaceUri") || jObject["NamespaceUri"].Type != JTokenType.String)
                    throw new Exception("The Object " + fieldName + " must have the string Property \"NamespaceUri\"");
                if (!jObject.ContainsKey("ServerIndex") || jObject["ServerIndex"].Type != JTokenType.Integer)
                    throw new Exception("The Object " + fieldName + " must have the integer Property \"ServerIndex\"");
            ...
            else
            {
                ...
                if (jtArray.GetElementsType() != JTokenType.Object)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array of JSON Object");
                JObject jObject;
                NodeId nodeId;
                ...
                while (newIterator.MoveNext())
                {
                    if (newIterator.Current.Name.Equals("opc:Field"))
                    {
                        ...
                        if (!dataToEncode.ContainsKey(fieldName))
                            throw new Exception("Wrong Object Properties: Expected a property named " + fieldName);
                        ...
                        if (!(typeName.Contains("opc:") || typeName.Contains("ua:")))
                        {
                            if (l == 1)
                            {
                                if (innerData.Type != JTokenType.Object)
                                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be a JSON Object");
                                EncodeJsonObject(typeName.Split(':')[1], innerData.ToObject<JObject>());
                            }
                            else
                            {
                                if (innerData.Type != JTokenType.Array)
                                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array");
                                JToken[] jtArray = innerData.Children().ToArray();
                                if (jtArray.Length != l)
                                    throw new Exception("Wrong Object Properties: The Array property named " + fieldName + " should be an Array with lenght = " + l);
                                if (jtArray.GetElementsType() != JTokenType.Object)
                                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array of JSON Object");
                                for (int i = 0; i < l; i++)
                                {
        ...
        private void EncodeLocalizedText(JToken jToken, string fieldName, int lenghtOfArray)
        {
            ...
            {
                if (jToken.Type != JTokenType.Object)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be a Object but received a " + jToken.Type.ToString());
                LocalizedText valueToWrite = null;
                JObject jObject = jToken.ToObject<JObject>();
                if (!jObject.ContainsKey("Locale") || !jObject.ContainsKey("Text"))
                    throw new Exception("The Object " + fieldName + " must have the string Properties \"Locale\" and \"Text\"");
            ...
            else
            {
                ...
                if (jtArray.Length != lenghtOfArray)
                    throw new Exception("Wrong Object Properties: The Array property named " + fieldName + " should be an Array with lenght = " + lenghtOfArray);
                if (jtArray.GetElementsType() != JTokenType.Object)
                    throw new Exception("Wrong Object Properties: The property named " + fieldName + " should be an Array of Object");
                ...
                for (int i = 0; i < lenghtOfArray; i++)
                {
                    jObject = jtArray[i].ToObject<JObject>();
                    if (!jObject.ContainsKey("Locale") || !jObject.ContainsKey("Text"))
                        throw new Exception("The elements of the Array Property " + fieldName + " must be object made up by the string Properties \"Locale\" and \"Text\"");
                    JToken jtLocale = jObject["Locale"];
                    JToken jtText = jObject["Text"];
                    if (jtLocale.Type != jtText.Type && jtLocale.Type != JTokenType.String)
                        throw new Exception("The elements of the Array Property " + fieldName + " must be object made up by the string Properties \"Locale\" and \"Text\"");
        ...
        private void EncodeQualifiedName(JToken jToken, string fieldName, int lenghtOfArray)
        {
            if (lenghtOfArray == 1)
            {
                ...
                if (!jObject.ContainsKey("Name") || !jObject.ContainsKey("NamespaceIndex"))
                    throw new Exception("The Object " + fieldName + " must have the Properties \"Name\" and \"NamespaceIndex\"");
                ...
                if (jtName.Type != JTokenType.String || jtNamespaceIndex.Type != JTokenType.Integer)
                    throw new Exception("The Object " + fieldName + " must have the string property \"Name\" and the integer property \"NamespaceIndex\"");
        ...
        private int LengthField(string lengthFieldSource, JObject currentJson)
        {
            ...
            if (!currentJson.ContainsKey(lengthFieldSource))
                throw new Exception("Wrong Object Properties: Expected a property named " + lengthFieldSource);
            if (currentJson[lengthFieldSource].Type != JTokenType.Integer)
                throw new Exception("Wrong Object Properties: The property named " + lengthFieldSource + " should be a JSON Integer");
