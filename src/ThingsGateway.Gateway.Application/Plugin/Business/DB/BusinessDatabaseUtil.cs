...
public static class BusinessDatabaseUtil
{
    ...
    public static async Task<OperResult<List<IDBHistoryValue>>> GetDBHistoryValuesAsync(string businessDeviceName, DBPageInput input)
    {
        try
        {
            var businessDevice = HostedServiceUtil.BusinessDeviceHostedService.DriverBases.Where(a => a is IDBHistoryService b).Where(a => a.DeviceName == businessDeviceName).FirstOrDefault();
