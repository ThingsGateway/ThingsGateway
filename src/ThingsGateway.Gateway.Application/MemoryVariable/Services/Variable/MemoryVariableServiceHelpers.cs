...
public static class MemoryVariableServiceHelpers
{
    public static USheetDatas ExportMemoryVariable(IEnumerable<MemoryVariable> variables, string sortName = nameof(MemoryVariable.Id), SortOrder sortOrder = SortOrder.Asc)
    {
...
IReadOnlyDictionary<long, ChannelRuntime> channelDicts)
    ...
    string? deviceName)
    {
        ...
        // 主变量页
        sheets.Add(GatewayExportString.VariableName, GetMemoryVariableSheets(data, deviceDicts, deviceName));
    ...
    static IEnumerable<Dictionary<string, object>> GetMemoryVariableSheets(
    IEnumerable<Variable> data,
    IReadOnlyDictionary<long, DeviceRuntime> deviceDicts,
    string? deviceName)
    {
#pragma warning disable CA1851 // “IEnumerable”集合可能的多个枚举
        var type = data.FirstOrDefault()?.GetType() ?? typeof(MemoryVariable);
        ...
        {
            yield return GetMemoryVariable(deviceDicts, deviceName, type, propertyInfos, variable);
        }
#pragma warning restore CA1851 // “IEnumerable”集合可能的多个枚举
    }

    private static Dictionary<string, object> GetMemoryVariable(IReadOnlyDictionary<long, DeviceRuntime> deviceDicts, string? deviceName, Type type, IOrderedEnumerable<PropertyInfo> propertyInfos, Variable variable)
    {
        ...
        {
            if (item.Name == nameof(MemoryVariable.Id))
            {
                continue;
            }
...
string? deviceName)
    ...
    public static Dictionary<string, object> ExportCore(IEnumerable<Variable> data, string deviceName = null, string sortName = nameof(MemoryVariable.Id), SortOrder sortOrder = SortOrder.Asc)
    {
        if (data?.Any() != true)
        {
            data = new List<MemoryVariable>();
        }
        var deviceDicts = GlobalData.ReadOnlyIdDevices;
        var channelDicts = GlobalData.ReadOnlyIdChannels;
        ...
        {
            {
                ...
                foreach (var item in propertyInfos)
                {
                    if (item.Name == nameof(MemoryVariable.Id))
                    {
                        if (sortName != nameof(MemoryVariable.Id))
                            continue;
                    }
                    //描述
                    var desc = type.GetPropertyDisplayName(item.Name);
        ...
        if (variableExports.FirstOrDefault()?.ContainsKey(sort) == false)
            sort = nameof(MemoryVariable.Id);
        if (variableExports.FirstOrDefault()?.ContainsKey(sort) == false)
            sort = type.GetPropertyDisplayName(nameof(MemoryVariable.Name));
        ...
        {
            devicePropertys[item] = new(devicePropertys[item].OrderBy(a => a[GatewayExportString.VariableName]));
    ...
    public static async Task<Dictionary<string, ImportPreviewOutputBase>> ImportAsync(USheetDatas uSheetDatas)
    {
        ...
        foreach (var sheetName in sheetNames)
        {
            ...
            deviceImportPreview = App.GetService<IMemoryVariableService>().SetVariableData(dataScope, deviceDicts, ImportPreviews, deviceImportPreview, driverPluginNameDict, propertysDict, sheetName, rows);
