...
public static class TypeExtension
{
    ...
    /// 外部添加获取属性描述的Func
    /// </summary>
    public static List<Func<MemberInfo, string>> DefaultDisplayNameFuncs { get; set; } = new();
    ...
    public static string FindDisplayAttribute(this MemberInfo memberInfo, Func<MemberInfo, string> func = null)
    {
        var dn = memberInfo.GetCustomAttribute<DisplayNameAttribute>(true)?.DisplayName
            ...
            ?? memberInfo.GetCustomAttribute<DisplayAttribute>(true)?.GetDescription()
            //?? memberInfo.GetCustomAttribute<SqlSugar.SugarColumn>(true)?.ColumnDescription
            ?? func?.Invoke(memberInfo);
        if (dn == null)
        {
            foreach (var defaultFunc in DefaultDisplayNameFuncs)
            {
    ...
    public static string? GetDescription(this Type modelType, string name, Func<MemberInfo, string> func = null)
    {
        ...
        var str = Cache.Default.GetOrAdd(cacheKey, entry =>
        {
            string dn = default;
            {
    ...
    {
        var cacheKey = $"{nameof(GetEnumList)}-{CultureInfo.CurrentUICulture.Name}-{type.FullName}-{func}-{type.TypeHandle.Value}";
        var str = Cache.Default.GetOrAdd(cacheKey, entry =>
        {
            {
                var enumType = type;
                ...
                for (int i = 0; i < fieldInfos.Count; i++)
                {
                    ...
                    if (fieldInfo.FieldType == enumType)
                    {
                        ...
                        list.Add(new(fieldInfo.Name, des, Enum.Parse(enumType, fieldInfo.Name)));
                    }
                }
                return list;
            }
...
{
    /// <inheritdoc/>
    public EnumItem(string name, string des, object value)
    {
        Name = name;
        Description = des;
        Value = value;
