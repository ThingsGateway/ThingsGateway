...
public class TextFileLogger : LoggerBase, IDisposable
{
        ...
        if (!_isFile && Backups > 0)
        {
            ...
            if (di.Exists)
            {
                ...
                if (fis != null && fis.Length > Backups)
                {
                    ...
                    {
                        WriteLog(LogLevel.Info, null, string.Format("Log file reached maximum limit {0}, delete {1}, size {2: n0} Byte", Backups, item.Name, item.Length), null);
                        try
                        {
                            item.Delete();
                        }
    ...
    /// TimeFormat
    /// </summary>
    public const string Format = "yyyy-MM-dd HH:mm:ss:ffff zz";
