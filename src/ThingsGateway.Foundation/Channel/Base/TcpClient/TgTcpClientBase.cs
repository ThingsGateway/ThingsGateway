...
namespace ThingsGateway.Foundation
{
    ...
    public class TgTcpClientBase : SetupConfigObject, ITcpClient
    {
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                lock (this.m_lock)
                {
                    if (this.m_online)
                    {
                        this.PrivateOnDisconnecting(new DisconnectEventArgs(true, DefaultResource.Localizer["ProactivelyDisconnect", nameof(Dispose)])).GetFalseAwaitResult();
            ...
            try
            {
                ...
                using (CancellationTokenSource cancellationTokenSource = new(timeout))
                {
                    if (!cancellationToken.CanBeCanceled)
                    {
                        ...
                        {
                            throw new TimeoutException(DefaultResource.Localizer["ConnectTimeout"]);
                        }
                    }
                    else
                    {
        ...
        public virtual void SetDataHandlingAdapter(SingleStreamDataHandlingAdapter adapter)
        {
            if (!this.CanSetDataHandlingAdapter)
            {
                throw new Exception(string.Format(DefaultResource.Localizer["CannotSet", nameof(SetDataHandlingAdapter)]));
            }
