...
/// 简单TCP客户端
/// </summary>
public class TGTcpClient : TGTcpClientBase
{
    /// <summary>
    /// 接收到数据
    /// </summary>
...
public class TGTcpClientBase : BaseSocket, ITcpClient
{
    /// <summary>
    /// 构造函数
    /// </summary>
    ...
    /// 异步连接服务器
    /// </summary>
    public async Task<ITcpClient> ConnectAsync(int timeout, CancellationToken cancellationToken)
    {
        try
        {
            if (this.CanSend || cancellationToken.IsCancellationRequested)
            {
                return this;
            }
            ...
            this.PrivateOnConnecting(args);
            {
                ...
                var task = this.MainSocket.ConnectAsync(iPHost.EndPoint, stoppingToken.Token).AsTask();
