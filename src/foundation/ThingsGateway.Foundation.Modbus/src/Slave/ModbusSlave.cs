...
public class ModbusSlave : ProtocolBase
{
    ...
    /// <summary>
    /// 是否校验/，对于Rtu来说是Crc，对于Tcp来说是事务检测符
    /// </summary>
...
.SpliceArray(new byte[] { (byte)1 });//01 lllegal function
    ...
    public override OperResult<byte[]> Read(string address, int length, CancellationToken cancellationToken = default)
    {
        try
        {
            ModbusAddress mAddress = ModbusAddressHelper.ParseFrom(address, this.Station);
            ...
            else
            {
                ...
                {
                    return new OperResult<byte[]>(string.Format(ModbusConst.StationNotSame, mAddress.Station, this.Station));
                }
                this.Init(mAddress);
            }
            ...
            return new OperResult<byte[]>(string.Format(ModbusConst.FunctionError));
        }
        catch (Exception ex)
        {
            return new(ex);
