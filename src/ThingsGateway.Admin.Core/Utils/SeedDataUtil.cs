...
public class SeedDataUtil
{
    public static List<T> GetSeedData<T>(string jsonName)
    {
        ...
        if (!string.IsNullOrEmpty(dataString))//如果有内容
        {
            ...
            //遍历seedDataRecord2
            for (var i = 0; i < seedDataRecord1.Records.Count; i++)
            {
                #region 处理ExtJosn
                ...
                {
                    //获取extjson的值
                    var extJson = propertyExtJosn.GetValue(seedDataRecord1.Records[i])?.ToString();
                    ...
                    if (!string.IsNullOrEmpty(extJson) && extJson.Contains("NullableDictionary"))
                    {
                        ...
                        propertyExtJosn.SetValue(seedDataRecord1.Records[i], extJson);
                    }
                }
                ...
                {
                    //获取extjson的值
                    var configValue = propertyConfigValue.GetValue(seedDataRecord1.Records[i])?.ToString();
                    // 如果extjson不为空并且包含NullableDictionary表示序列化失败了
                    if (!string.IsNullOrEmpty(configValue) && configValue.Contains("NullableDictionary"))
                    {
                        ...
                        //seedDataRecord2赋值seedDataRecord1的extjson
                        propertyConfigValue.SetValue(seedDataRecord1.Records[i], configValue);
                    }
                }
