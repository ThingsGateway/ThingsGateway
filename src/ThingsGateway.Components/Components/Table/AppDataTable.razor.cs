    ...
    where EditItem : class, new()
{
    ...
    /// </summary>
    [Parameter]
    public StringNumber? TableHeight { get; set; } = null;
    ...
    private SqlSugarPagedList<TItem> PageItems { get; set; } = new SqlSugarPagedList<TItem>();
    ...
    public async Task QueryClickAsync()
    {
        try
        {
            ...
            {
                if (SearchModel.Current > PageItems.Pages)
                {
    ...
    /// </summary>
    [Parameter]
    public bool FirstQuery { get; set; } = true;
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        if (IsShowOperCol)
        {
            Headers.Add(new()
            {
                Text = AppService.I18n.T("操作"),
                Value = BlazorAppService.DataTableActions,
                Width = 200,
        ...
        //定义操作列宽度
        var action = Headers.FirstOrDefault(a => a.Value == BlazorAppService.DataTableActions);
        if (action != null)
    ...
    private async Task DeleteClick(params TItem[] _selectedItem)
    {
        ...
        try
        {
            ...
            {
                await PopupService.EnqueueSnackbarAsync(AppService.I18n.T("选择一行后才能进行操作"));
            }
            else
            {
                if (DeleteCallAsync != null)
                {
                    var confirm = await PopupService.ConfirmAsync(AppService.I18n.T("删除"), AppService.I18n.T("确定?"), AlertTypes.Warning);
                    if (confirm)
                    {
    ...
    private async Task DetailClick(params TItem[] _selectedItem)
    {
        ...
        {
            await PopupService.EnqueueSnackbarAsync(AppService.I18n.T("只能选择一行"));
        }
        else if (_selectedItem.Length == 1)
        {
            ...
            {
                if (item != BlazorAppService.DataTableActions)
                {
