...
internal class UpdateZipFileService : IUpdateZipFileService, IDisposable
{
    ...
    public async Task Update(UpdateZipFile updateZipFile, Func<Task<bool>> check = null)
    {
        try
        {
            if (WaitLock.IsWaitting)
            {
                throw new("正在更新中，请稍后再试");
            }
            await WaitLock.WaitAsync();
            RestartServerHelper.DeleteAndBackup();
            using var client = await GetTcpDmtpClient();

            var result = await ClientPullFileFromService(client, updateZipFile.FilePath);
            if (result)
            {
                if (check != null)
                    result = await check.Invoke();
                if (result)
                {
    ...
    /// 客户端从服务器下载文件。
    /// </summary>
    private async Task<bool> ClientPullFileFromService(TcpDmtpClient client, string path)
    {
    ...
    /// 客户端上传文件到服务器。
    /// </summary>
    private async Task ClientPushFileFromService(TcpDmtpClient client, string serverPath, string resourcePath)
    {
        var metadata = new Metadata();//传递到服务器的元数据
