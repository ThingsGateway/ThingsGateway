...
internal class RedundancyService : BaseService<SysDict>, IRedundancyService
{
    ...
    public async Task EditRedundancyOptionAsync(RedundancyOptions input)
    {
        ...
        List<SysDict> dicts = new List<SysDict>()
        {
            ...
            new SysDict() { DictType = DictTypeEnum.System, Category = nameof(RedundancyOptions), Name = nameof(RedundancyOptions.PrimaryUri), Code = input.PrimaryUri.ToString() },
            new SysDict() { DictType = DictTypeEnum.System, Category = nameof(RedundancyOptions), Name = nameof(RedundancyOptions.IsPrimary), Code = input.IsPrimary.ToString() },
    ...
    public async Task<RedundancyOptions> GetRedundancyAsync()
    {
        ...
        if (redundancy == null)
        {
            ...
            redundancy.Enable = sysDicts.FirstOrDefault(a => a.Category == nameof(RedundancyOptions) && a.Name == nameof(RedundancyOptions.Enable))?.Code.ToBoolean() ?? true;
            redundancy.PrimaryUri = sysDicts.FirstOrDefault(a => a.Category == nameof(RedundancyOptions) && a.Name == nameof(RedundancyOptions.PrimaryUri))?.Code.ToString();
            redundancy.IsPrimary = sysDicts.FirstOrDefault(a => a.Category == nameof(RedundancyOptions) && a.Name == nameof(RedundancyOptions.IsPrimary))?.Code.ToBoolean() ?? true;
            ...
            redundancy.IsStartBusinessDevice = sysDicts.FirstOrDefault(a => a.Category == nameof(RedundancyOptions) && a.Name == nameof(RedundancyOptions.IsStartBusinessDevice))?.Code.ToBoolean() ?? true;
