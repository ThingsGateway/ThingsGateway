        ...
        private async Task LoginAsync()
        {
            ...
            if (str != null)
            {
                ...
                if (ret.Code != 200)
                {
                    ...
                    await PopupService.EnqueueSnackbarAsync(T("登录错误") + ": " + ret.Msg.ToString(), AlertTypes.Error);
                }
                else
                {
                    await PopupService.EnqueueSnackbarAsync(T("登录成功"), AlertTypes.Success);
            ...
            {
                await captcha.RefreshCode();
                await PopupService.EnqueueSnackbarAsync(@T("登录错误"), AlertTypes.Error);
            }
        }
