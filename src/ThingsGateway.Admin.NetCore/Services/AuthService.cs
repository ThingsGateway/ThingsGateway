...
public class AuthService : IAuthService
{
    ...
    {
        var key = ThingsGateway.Admin.Application.CacheConst.Cache_LoginErrorCount + userName;//获取登录错误次数Key值
    ...
    private async Task<LoginOutput> ExecLogin(LoginPolicy loginPolicy, LoginInput input, SysUser sysUser)
    {
        ...
        string refreshToken = string.Empty;
        {
            ...
            var identity = new ClaimsIdentity(nameof(ThingsGateway));
            ...
            _appService.User = new ClaimsPrincipal(identity);
    ...
    private async Task UpdateUser(LoginEvent loginEvent)
    {
        ...
        var key = ThingsGateway.Admin.Application.CacheConst.Cache_LoginErrorCount + sysUser.Account;//获取登录错误次数Key值
