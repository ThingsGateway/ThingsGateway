...
public partial class MemoryVariableEditComponent
{
    /// <summary>
    /// IntFormatter
    /// </summary>
    ...
    [EditorRequired]
    [NotNull]
    public MemoryVariableRuntime? Model { get; set; }
    ...
    [Inject]
    IStringLocalizer<MemoryVariable> VariableLocalizer { get; set; }

    [Inject]
    IVariablePageService VariablePageService { get; set; }
    private async Task ShowExpressionsUI(MemoryVariableRuntime Model, bool read)
    {
        string script = null;
        if (read)
        {
            script = Model.ReadExpressions;
        }
        else
        {
            script = Model.WriteExpressions;
        }
        ...
        var op = new DialogOption()
        {
            ...
            ShowMaximizeButton = true,
            Size = Size.ExtraLarge,
            Title = $"{Model.Name} {(read ? VariableLocalizer[nameof(MemoryVariable.ReadExpressions)] : VariableLocalizer[nameof(MemoryVariable.WriteExpressions)])}",
            ShowFooter = false,
            ShowCloseButton = false,
            BodyTemplate = BootstrapDynamicComponent.CreateComponent<MemoryVariableScriptCheck>(new Dictionary<string, object?>
    {
        {nameof(MemoryVariableScriptCheck.Script),script },
        {nameof(MemoryVariableScriptCheck.GetResult), async (string script)=>
        {
                StringBuilder stringBuilder=new();
               var logger=new TouchSocket.Core.EasyLogger((a)=>stringBuilder.AppendLine(a));
         var data=   MemoryExpressionEvaluatorExtension.GetMemoryReadWriteExpressions(script);
            data.Log=new (logger);
               foreach (var item in data.Tags)
    {
                if(await VariablePageService.GetVariableAsync(item.Item1,item.Item2 )==null)
                {
                    logger.LogWarning($"can not found device:{item.Item1}, variable:{item.Item2}");
                }
    }
#if !Management
            var value = data.GetNewValue();
                stringBuilder.AppendLine("result: "+value.ToSystemTextJsonString());
#else
        ...
        {nameof(MemoryVariableScriptCheck.OnGetDemo),()=>
                {
                    return """
         ...
         {nameof(MemoryVariableScriptCheck.ScriptChanged),EventCallback.Factory.Create<string>(this, a =>
        {
                 if(read)
                 {
