...
public static class StartupExtensions
{
    ...
    private static void AddStartups(WebApplicationBuilder builder)
    {
        ...
        if (builder.Environment is IWebHostEnvironment webHostEnvironment)
            NetCoreApp.WebRootPath = webHostEnvironment.WebRootPath;
        NetCoreApp.ContentRootPath = builder.Environment.ContentRootPath;
        NetCoreApp.IsDevelopment = builder.Environment.IsDevelopment();
    ...
    {
        NetCoreApp.RootServices = app.ApplicationServices;
        NetCoreApp.CacheService = app.ApplicationServices.GetRequiredService<ICacheService>();
        ...
        foreach (var startup in startups)
        {
            ...
            foreach (var method in configureMethods)
            {
                method.Invoke(startup, ResolveMethodParameterInstances(NetCoreApp.RootServices, method));
            }
        }
        AppStartups.Clear();
    }
