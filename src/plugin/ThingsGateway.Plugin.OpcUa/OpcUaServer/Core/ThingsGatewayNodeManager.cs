...
public class ThingsGatewayNodeManager : CustomNodeManager2
{
    ...
    public override void CreateAddressSpace(IDictionary<NodeId, IList<IReference>> externalReferences)
    {
        lock (Lock)
        {
            ...
            //创建设备树
            var _geviceGroup = _businessBase.CurrentDevice.VariableRunTimes.Select(a => a.Value)
        ...
        try
        {
            ...
            if (NodeIdTags.TryGetValue(variable.SymbolicName, out OpcUaTag tag))
            {
                if (StatusCode.IsGood(variable.StatusCode))
                {
                    ...
                    {
                        var result = _businessBase.RpcService.InvokeDeviceMethodAsync("OpcUaSlave - " + context1?.OperationContext?.Session?.Identity?.DisplayName,
                            new()
                            {
