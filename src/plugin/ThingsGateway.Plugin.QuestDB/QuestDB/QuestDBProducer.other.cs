...
public partial class QuestDBProducer : BusinessBaseWithCacheInterval<QuestDBHistoryValue>
{
    ...
    {
        AddQueueT(new(variableRunTime.Adapt<QuestDBHistoryValue>()));
        base.VariableChange(variableRunTime);
    }

    protected override Task<OperResult> UpdateT(IEnumerable<LiteDBDefalutCacheItem<QuestDBHistoryValue>> item, CancellationToken cancellationToken)
    {
        return UpdateT(item.Select(a => a.Data), cancellationToken);
    }

    private async Task<OperResult> UpdateT(IEnumerable<QuestDBHistoryValue> item, CancellationToken cancellationToken)
    {
