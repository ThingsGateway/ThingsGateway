...
public class AjaxService
{
    ...
    /// 获得 下载委托缓存集合
    /// </summary>
    private List<(IComponent Key, Func<string, string, object, Task> Callback)> DownFileCache { get; } = new();
    ...
    {
        var cb = DownFileCache.FirstOrDefault().Callback;
        if (cb != null)
        {
            await cb.Invoke(url, fileName, dtoObject);
        }
    }
    /// <summary>
    /// 调用 Goto 方法跳转其他页面
    ...
    /// <param name="key"></param>
    /// <param name="callback"></param>
    internal void RegisterDownFile(IComponent key, Func<string, string, object, Task> callback) => DownFileCache.Add((key, callback));
    ...
    /// 注销事件
    /// </summary>
    internal void UnRegisterDownFile(IComponent key)
    {
        var item = DownFileCache.FirstOrDefault(i => i.Key == key);
        if (item.Key != null)
        {
            DownFileCache.Remove(item);
        }
    }
}