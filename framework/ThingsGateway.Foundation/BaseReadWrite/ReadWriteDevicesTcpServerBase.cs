...
public abstract class ReadWriteDevicesTcpServerBase : ReadWriteDevicesClientBase
{
    ...
    /// 接收解析
    /// </summary>
    protected abstract void Received(SocketClient client, IRequestInfo requestInfo);

    private void Connected(SocketClient client, ConnectedEventArgs e)
    {
        Logger?.Debug(client.IP + ":" + client.Port + "连接成功");
    }

    private void Connecting(SocketClient client, ConnectingEventArgs e)
    {
