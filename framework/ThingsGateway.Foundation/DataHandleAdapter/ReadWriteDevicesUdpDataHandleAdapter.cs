...
public abstract class ReadWriteDevicesUdpDataHandleAdapter<TRequest> : UdpDataHandlingAdapter where TRequest : class, IMessage
{
    ...
    protected void GoSend(EndPoint endPoint, byte[] item)
    {
        ...
        Logger?.Trace(FoundationConst.LogMessageHeader + ToString() + " - " + ThingsGatewayStatus.Send.GetDescription() + ":" + Request.SendBytes.ToHexString(" "));
    }
    /// <inheritdoc/>
    protected override void PreviewReceived(EndPoint remoteEndPoint, ByteBlock byteBlock)
    {
        ...
        Logger?.Trace(FoundationConst.LogMessageHeader + ToString() + " - " + ThingsGatewayStatus.Received.GetDescription() + ":" + allBytes.ToHexString(" "));
