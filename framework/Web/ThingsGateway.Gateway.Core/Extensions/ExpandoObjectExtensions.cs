...
public static class ExpandoObjectExtensions
{
    ...
    {
        var entity = new T();
        var properties = typeof(T).GetPropertiesWithCache().Where(a => !filter || a.GetCustomAttribute<IgnoreExcelAttribute>() == null).ToDictionary(a => a.FindDisplayAttribute());
        ...
        expandoObject.ForEach(keyValuePair =>
        {
            if (properties.TryGetValue(keyValuePair.Key, out var property))
            {
                ...
                var objValue = property.PropertyType.ObjToTypeValue(value?.ToString() ?? "");
    ...
    {
        var properties = typeof(T).GetPropertiesWithCache();
