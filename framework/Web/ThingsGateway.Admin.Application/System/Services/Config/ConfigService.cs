...
public class ConfigService : DbRepository<SysConfig>, IConfigService
{
    ...
    public async Task<List<SysConfig>> GetListByCategoryAsync(string category)
    {
        ...
        var configList = Cache.SysMemoryCache.Get<List<SysConfig>>(CacheConst.SYS_CONFIGCATEGORY + category, true);
        if (configList == null)
        {
            //cache没有再去数据可拿
            ...
            {
                Cache.SysMemoryCache.Set(CacheConst.SYS_CONFIGCATEGORY + category, configList, true);//如果不为空,插入cache
            }
        }
        return configList;
    }
    ...
    {
        Cache.SysMemoryCache.Remove(CacheConst.SYS_CONFIGCATEGORY + category);//cache删除
    }
