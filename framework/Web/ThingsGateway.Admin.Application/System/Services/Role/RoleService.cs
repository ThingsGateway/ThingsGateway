...
namespace ThingsGateway.Admin.Application
{
    ...
    public class RoleService : DbRepository<SysRole>, IRoleService
    {
        ...
        public override async Task<List<SysRole>> GetListAsync()
        {
            ...
            var sysRoles = Cache.SysMemoryCache.Get<List<SysRole>>(CacheConst.CACHE_SYSROLE, true);
            if (sysRoles == null)
            {
                //cache没有就去数据库拿
                ...
                {
                    //插入Cache
                    Cache.SysMemoryCache.Set(CacheConst.CACHE_SYSROLE, sysRoles, true);
                }
            }
            return sysRoles;
        }
        ...
        {
            Cache.SysMemoryCache.Remove(CacheConst.CACHE_SYSROLE);//删除KEY
        }
