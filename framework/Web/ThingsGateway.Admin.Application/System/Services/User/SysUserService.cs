...
public class SysUserService : DbRepository<SysUser>, ISysUserService
{
    ...
    public void DeleteUserFromCache(params long[] ids)
    {
        ...
        {
            var user = Cache.SysMemoryCache.Get<SysUser>(CacheConst.CACHE_SYSUSER + item, false);//获取用户列表
            sysUsers.Add(user);
            //删除账号
            Cache.SysMemoryCache.Remove(CacheConst.CACHE_SYSUSER + item);
        }
        ...
        if (sysUsers.Count > 0)
        {
            ...
            {
                //删除账号
                Cache.SysMemoryCache.Remove(CacheConst.CAHCE_SYSUSERACCOUNT + item);
            }
        }
    }
    ...
    {
        //先从Cache拿
        var userId = Cache.SysMemoryCache.Get<long>(CacheConst.CAHCE_SYSUSERACCOUNT + account, false);
        if (userId == 0)
        {
            //单查获取用户账号对应ID
            ...
            {
                //插入Cache
                Cache.SysMemoryCache.Set(CacheConst.CAHCE_SYSUSERACCOUNT + account, userId, false);
            }
        }
        return userId;
    }
    ...
    public async Task<SysUser> GetUserByIdAsync(long Id)
    {
        ...
        var sysUser = Cache.SysMemoryCache.Get<SysUser>(CacheConst.CACHE_SYSUSER + Id.ToString(), true);
        if (sysUser == null)
        {
            ...
            if (sysUser != null)
            {
                ...
                //插入Cache
                Cache.SysMemoryCache.Set(CacheConst.CACHE_SYSUSER + sysUser.Id.ToString(), sysUser, true);
            }
        }
        return sysUser;
    }
