...
public class RelationService : DbRepository<SysRelation>, IRelationService
{
    ...
    public async Task<List<SysRelation>> GetRelationByCategoryAsync(string category)
    {
        ...
        var sysRelations = Cache.SysMemoryCache.Get<List<SysRelation>>(CacheConst.CACHE_SYSRELATION + category, true);
        if (sysRelations == null)
        {
            ...
            {
                //插入Cache
                Cache.SysMemoryCache.Set(CacheConst.CACHE_SYSRELATION + category, sysRelations, true);
            }
        }
        return sysRelations;
    }
    ...
    {
        Cache.SysMemoryCache.Remove(CacheConst.CACHE_SYSRELATION + category);//删除cache
    }
