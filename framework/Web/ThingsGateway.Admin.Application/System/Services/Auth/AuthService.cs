...
public class AuthService : IAuthService
{
    ...
    public ValidCodeOutput GetCaptchaInfo()
    {
        ...
        //插入cache
        Cache.SysMemoryCache.Set(CacheConst.LOGIN_CAPTCHA + reqNo, captchInfo, TimeSpan.FromMinutes(1), false);
        //返回验证码和请求号
    ...
    {
        var code = Cache.SysMemoryCache.Get<string>(CacheConst.LOGIN_CAPTCHA + validCodeReqNo, false);//从cache拿数据
        if (isDelete) Cache.SysMemoryCache.Remove(CacheConst.LOGIN_CAPTCHA + validCodeReqNo);//删除验证码
        if (code != null)//如果有
        {
