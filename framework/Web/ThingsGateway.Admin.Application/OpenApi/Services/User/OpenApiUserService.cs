...
public class OpenApiUserService : DbRepository<OpenApiUser>, IOpenApiUserService
{
    ...
    public void DeleteUserFromCache(params long[] ids)
    {
        ...
        {
            var user = Cache.SysMemoryCache.Get<OpenApiUser>(CacheConst.CACHE_OPENAPIUSER + item, false);//获取用户列表
            openApiUsers.Add(user);
            Cache.SysMemoryCache.Remove(CacheConst.CACHE_OPENAPIUSER + item);
        }
        ...
        if (openApiUsers.Count > 0)
        {
            ...
            {
                //删除账号
                Cache.SysMemoryCache.Remove(CacheConst.CACHE_OPENAPIUSERACCOUNT + item);
            }
        }
    }
    ...
    {
        //先从Cache拿
        var userId = Cache.SysMemoryCache.Get<long>(CacheConst.CACHE_OPENAPIUSERACCOUNT + account, false);
        if (userId == 0)
        {
            //单查获取用户账号对应ID
            ...
            {
                //插入Cache
                Cache.SysMemoryCache.Set(CacheConst.CACHE_OPENAPIUSERACCOUNT + account, userId, false);
            }
        }
        return userId;
    }
    ...
    public async Task<OpenApiUser> GetUsertByIdAsync(long Id)
    {
        ...
        var openApiUser = Cache.SysMemoryCache.Get<OpenApiUser>(CacheConst.CACHE_OPENAPIUSER + Id.ToString(), true);
        if (openApiUser == null)
        {
            ...
            {
                //插入Cache
                Cache.SysMemoryCache.Set(CacheConst.CACHE_OPENAPIUSER + openApiUser.Id.ToString(), openApiUser, true);
            }
        }
        return openApiUser;
    }
