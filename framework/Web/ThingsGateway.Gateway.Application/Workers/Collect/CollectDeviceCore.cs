...
public class CollectDeviceCore
{
    ...
    /// </summary>
    /// <returns></returns>
    internal async Task BeforeActionAsync(CancellationToken cancellationToken, object client = null)
    {
        try
        {
    ...
    internal async Task<ThreadRunReturn> RunActionAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (_driver.IsSupportRequest)
            {
                ...
                foreach (var deviceVariableMethodRead in DeviceVariableMethodReads)
                {
                    ...
                    if (deviceVariableMethodRead.CheckIfRequestAndUpdateTime(DateTimeExtensions.CurrentDateTime))
                    {
                        ...
                        var read = await InvokeMethodAsync(deviceVariableMethodRead, cancellationToken);
                        if (read.IsSuccess)
                        {
            ...
            else //插件自更新设备状态与变量状态
            {
                ...
                {
                    //更新设备活动时间
                    Device.SetDeviceStatus(DateTimeExtensions.CurrentDateTime, errorCount: 0);
                }
                else
                {
    ...
    /// 执行特殊方法
    /// </summary>
    internal async Task<OperResult<string>> InvokeMethodAsync(DeviceVariableMethodSource deviceVariableMethodSource, bool isRead, string value, CancellationToken cancellationToken)
    {
        try
        {
            await easyLock.WaitAsync();
    ...
    /// 执行轮询特殊方法,并设置变量值
    /// </summary>
    private async Task<OperResult<string>> InvokeMethodAsync(DeviceVariableMethodSource deviceVariableMethodRead, CancellationToken cancellationToken)
    {
        var data = await InvokeMethodAsync(deviceVariableMethodRead, true, string.Empty, cancellationToken);
        return data;
    }
