...
public class CollectDeviceWorker : BackgroundService
{
    ...
    /// <inheritdoc/>
    public CollectDeviceWorker(ILogger<CollectDeviceWorker> logger, IHostApplicationLifetime appLifetime)
    {
        _logger = logger;
    ...
    private async Task CreatAllDeviceThreadsAsync()
    {
        if (!_stoppingToken.IsCancellationRequested)
        {
            ...
            var collectDeviceRunTimes = (await App.GetService<ICollectDeviceService>().GetCollectDeviceRuntimeAsync());
    ...
    {
        await CollectDeviceThreads.ParallelForEachAsync(async (deviceThread, cancellationToken) =>
        {
            try
            {
    ...
    private async Task StartOtherHostService()
    {
        ...
        var historyValueService = BackgroundServiceUtil.GetBackgroundService<HistoryValueWorker>();
        var uploadDeviceHostService = BackgroundServiceUtil.GetBackgroundService<UploadDeviceWorker>();
        var memoryVariableWorker = BackgroundServiceUtil.GetBackgroundService<MemoryVariableWorker>();
        ...
        try
        {
            ...
            if (!_stoppingToken.IsCancellationRequested)
            {
                ...
                if (dev.IsRedundant)
                {
                    ...
                    {
                        var newDev = (await App.GetService<ICollectDeviceService>().GetCollectDeviceRuntimeAsync(devId)).FirstOrDefault();
                        if (dev == null)
                        {
                    ...
                    else
                    {
                        try
                        {
                            var Redundantdev = (await App.GetService<ICollectDeviceService>().GetCollectDeviceRuntimeAsync(dev.RedundantDeviceId)).FirstOrDefault();
    ...
    {
        var driverPluginService = App.GetService<IDriverPluginService>();
        var driverId = App.GetService<ICollectDeviceService>().GetDeviceById(devId).PluginId;
        var driverPlugin = driverPluginService.GetDriverPluginById(driverId);
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        || (devcore.IsInitSuccess == false && devcore.Device.ActiveTime.AddMinutes(10) <= DateTimeExtensions.CurrentDateTime)
                        ...
                        {
                            if (devcore.Device.IsRedundant && App.GetService<ICollectDeviceService>().GetCacheList(false).Any(a => a.Id == devcore.Device.RedundantDeviceId))
                            {
