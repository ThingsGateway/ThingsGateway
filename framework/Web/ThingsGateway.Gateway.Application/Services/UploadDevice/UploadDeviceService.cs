...
public class UploadDeviceService : DbRepository<UploadDevice>, IUploadDeviceService
{
    ...
    {
        //先从Cache拿
        var uploadDevice = Cache.SysMemoryCache.Get<List<UploadDevice>>(ThingsGatewayCacheConst.Cache_UploadDevice, isMapster);
        if (uploadDevice == null)
        {
            ...
            {
                //插入Cache
                Cache.SysMemoryCache.Set(ThingsGatewayCacheConst.Cache_UploadDevice, uploadDevice, isMapster);
            }
        }
        return uploadDevice;
    }
