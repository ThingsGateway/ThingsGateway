...
public class CollectDeviceService : DbRepository<CollectDevice>, ICollectDeviceService
{
    ...
    {
        //先从Cache拿
        var collectDevice = Cache.SysMemoryCache.Get<List<CollectDevice>>(ThingsGatewayCacheConst.Cache_CollectDevice, isMapster);
        if (collectDevice == null)
        {
            ...
            {
                //插入Cache
                Cache.SysMemoryCache.Set(ThingsGatewayCacheConst.Cache_CollectDevice, collectDevice, isMapster);
            }
        }
