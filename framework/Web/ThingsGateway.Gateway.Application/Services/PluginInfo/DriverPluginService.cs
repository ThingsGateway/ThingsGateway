...
public partial class DriverPluginService : DbRepository<DriverPlugin>, IDriverPluginService
{
    ...
    public async Task AddAsync(DriverPluginAddInput input)
    {
        ...
        var datas = await pluginService.TryAddDriverAsync(input);
        ...
        {
            Cache.SysMemoryCache.Remove(ThingsGatewayCacheConst.Cache_DriverPlugin);//cache删除
        }
        else
        {
    ...
    {
        //先从Cache拿
        var driverPlugins = Cache.SysMemoryCache.Get<List<DriverPlugin>>(ThingsGatewayCacheConst.Cache_DriverPlugin, isMapster);
        if (driverPlugins == null)
        {
            ...
            {
                //插入Cache
                Cache.SysMemoryCache.Set(ThingsGatewayCacheConst.Cache_DriverPlugin, driverPlugins, isMapster);
            }
        }
