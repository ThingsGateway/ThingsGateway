...
public partial class Oplog
{
    ...
    private Task<ISqlSugarPagedList<SysOperateLog>> QueryCallAsync(OperateLogPageInput input)
    {
    ...
    async Task DownExportAsync(OperateLogPageInput input = null)
    {
        try
        {
            await AjaxService.DownFileAsync("file/operateLog", DateTimeExtensions.CurrentDateTime.ToFileDateTimeFormat(), input.Adapt<OperateLogInput>());
        }
        finally
        {
        }
