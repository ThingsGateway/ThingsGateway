...
public class UserCenterService : DbRepository<SysUser>, IUserCenterService
{
    ...
    public async Task<List<long>> GetLoginWorkbenchAsync()
    {
        ...
        {
            //如果有数据直接返回个人工作台
            return sysRelation.ExtJson.ToJsonWithT<List<long>>();
        }
        else
        {
            return new();
    ...
    public async Task<long> GetLoginDefaultRazorAsync(long userId)
    {
        ...
        if (result != null)
            return result.ExtJson.ToJsonWithT<long>();
        else
            return 0;
    }
    /// <inheritdoc />
    public Task UpdateUserDefaultRazorAsync(long userId, long defalutRazor)
    {
        return _relationService.SaveRelationAsync(CateGoryConst.Relation_SYS_USER_DEFAULTRAZOR, userId, null, defalutRazor.ToJsonString(), true);
    }
