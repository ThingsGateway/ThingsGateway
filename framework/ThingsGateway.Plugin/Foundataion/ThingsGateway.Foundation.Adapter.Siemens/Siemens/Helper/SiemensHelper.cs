...
internal partial class SiemensHelper
{
    ...
    internal static async Task<OperResult<string>> ReadStringAsync(SiemensS7PLC plc, string address, Encoding encoding, CancellationToken token)
    {
        //先读取一次获取长度，再读取实际值
        if (plc.CurrentPlc != SiemensEnum.S200Smart)
        {
            var result1 = await plc.ReadAsync(address, 2, token);
        ...
        {
            var result1 = await plc.ReadAsync(address, 1, token);
            if (!result1.IsSuccess)
                return result1.Copy<string>();
            var result2 = await plc.ReadAsync(address, 1 + result1.Content[0], token);
            if (!result2.IsSuccess)
            {
                return result2.Copy<string>();
            }
