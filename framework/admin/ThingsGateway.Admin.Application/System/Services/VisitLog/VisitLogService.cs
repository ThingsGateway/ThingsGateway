...
public class VisitLogService : DbRepository<SysVisitLog>, IVisitLogService
{
    ...
    /// <inheritdoc />
    public async Task<ISqlSugarPagedList<SysVisitLog>> PageAsync(VisitLogPageInput input)
    {
    ...
    public async Task<MemoryStream> ExportFileAsync(List<SysVisitLog> input = null)
    {
    ...
    /// <inheritdoc/>
    [OperDesc("导出访问日志", IsRecordPar = false)]
    public async Task<MemoryStream> ExportFileAsync(VisitLogInput input)
    {
        var query = GetPage(input.Adapt<VisitLogPageInput>());
