...
internal static class PackHelper
{
    ...
    /// <returns></returns>
    public static List<T> LoadSourceRead<T, T2>(SiemensS7PLC device, List<T2> deviceVariables, int maxPack, int defaultIntervalTime) where T : IDeviceVariableSourceRead<IDeviceVariableRunTime>, new() where T2 : IDeviceVariableRunTime, new()
    {
        ...
        foreach (var item in tags)
        {
            Dictionary<SiemensAddress, T2> map = item.ToDictionary(it =>
            {
                try
                {
                    var s7Address = SiemensAddress.ParseFrom(it.Address);
                    ...
                    if (lastLen <= 0)
                    {
                        switch (it.DataTypeEnum)
                        {
                            case DataTypeEnum.String:
                                ...
                                {
                                    if (device.SiemensEnum == SiemensEnum.S200Smart)
                                    {
            ...
            foreach (var functionCode in functionCodes)
            {
                ...
                foreach (var stationNumber in stationNumbers)
                {
                    ...
                    var tempResult = LoadSourceRead<T, T2>(addressList, functionCode, item.Key, device);
                    //添加到总连读包
    ...
    private static List<T> LoadSourceRead<T, T2>(Dictionary<SiemensAddress, T2> addressList, int functionCode, int intervalTime, SiemensS7PLC siemensS7Net) where T : IDeviceVariableSourceRead<IDeviceVariableRunTime>, new() where T2 : IDeviceVariableRunTime, new()
    {
        ...
        while (maxAddress >= minAddress)
        {
            ...
            T sourceRead = new()
            {
                IntervalTimeTick = new(intervalTime),
